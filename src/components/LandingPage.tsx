...
const LandingPage = ({ onRunReport, isSearching, appState }: LandingPageProps) => {
  ...
  return (
    <div id="landing-scroll-container" className="fixed inset-0 overflow-y-auto overflow-x-hidden bg-paper z-[100]" style={{ scrollbarWidth: "thin" }}>
      {/* Sticky nav */}
      <nav
        className={`fixed top-0 left-0 right-0 z-[200] flex items-center justify-between px-12 py-5 transition-all duration-300 ${
          scrolled ? "bg-paper/[0.92] backdrop-blur-md border-b border-ink/10" : "border-b border-transparent"
        }`}
      >
        <div className="font-display text-[20px] font-bold tracking-tight text-ink" style={{ letterSpacing: "-0.025em" }}>
          Teth<span className="text-red">y</span>r
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => scrollTo("how-it-works")} className="font-mono text-[10px] tracking-[0.1em] uppercase text-ink-3 hover:text-ink cursor-pointer bg-transparent border-none transition-colors">
            How it works
          </button>
          <button onClick={() => scrollTo("sources")} className="font-mono text-[10px] tracking-[0.1em] uppercase text-ink-3 hover:text-ink cursor-pointer bg-transparent border-none transition-colors">
            Sources
          </button>
          <button onClick={() => scrollTo("pricing")} className="font-mono text-[10px] tracking-[0.1em] uppercase text-ink-3 hover:text-ink cursor-pointer bg-transparent border-none transition-colors">
            Pricing
          </button>
          <button
            onClick={() => scrollTo("hero-input")}
            className="bg-ink text-paper px-[18px] py-[9px] rounded-sm font-mono text-[10px] tracking-[0.1em] uppercase border-none cursor-pointer hover:bg-red transition-colors ml-1"
          >
            Start free →
          </button>
        </div>
      ...
      {/* Section 1: Hero */}
      <section className="relative min-h-screen flex flex-col items-center justify-center px-12 pt-[100px] pb-[80px] z-[1] overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none select-none" aria-hidden="true">
          <span className="font-display font-bold text-ink/[0.03] whitespace-nowrap" style={{ fontSize: "clamp(120px, 20vw, 260px)", letterSpacing: "-0.04em" }}>
            TRUTH
          </span>
        </div>
        ...
        <div className="relative z-10 text-center max-w-[900px] mx-auto">
          ...
          <div className="flex items-center justify-center gap-3 mb-6">
            <div className="w-10 h-px bg-ink/[0.22]" />
          ...
          {/* Headline */}
          <h1 className="font-display font-normal leading-[1.1] mb-3" style={{ fontSize: "clamp(44px, 6vw, 80px)", letterSpacing: "-0.025em" }}>
            Stop building what <em className="italic text-red">AI thinks</em>
            <br />
            people need.
          </h1>
          <p className="font-display italic leading-[1.1] text-ink-3 mb-7" style={{ fontSize: "clamp(44px, 6vw, 80px)", letterSpacing: "-0.025em" }}>
            Start building what they actually say.
          </p>

          {/* Subtitle */}
          <p className="font-body text-[18px] text-ink-3 leading-[1.7] font-light max-w-[520px] mx-auto mb-12">
        ...
        {/* Scroll hint */}
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5 opacity-50 animate-scroll-bounce">
          <span className="font-mono text-[8px] tracking-[0.14em] uppercase text-ink-4">Scroll to learn more</span>
          <div className="w-px h-6 bg-ink-4" />
        </div>
      </section>

      {/* Section 2: The Problem */}
      <section ref={s2.ref} className={`py-[100px] px-12 max-w-[1100px] mx-auto relative z-[1] ${s2.className}`}>
        <SectionRule num="01" title="The Problem" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[60px] items-start">
          <div>
            <h2 className="font-display font-normal leading-[1.2] mb-6" style={{ fontSize: "clamp(32px, 3.5vw, 48px)", letterSpacing: "-0.02em" }}>
              Every AI tool tells you your idea is <em className="italic text-red">great.</em>
            </h2>
            <p className="font-body text-[17px] text-ink-3 leading-[1.75] font-light mb-5">
            ...
            <p className="font-body text-[17px] text-ink-3 leading-[1.75] font-light">
          ...
          <div className="border border-ink/[0.22]">
            {[
              ...
              <div key={i} className={`px-7 py-6 ${i < 2 ? "border-b border-ink/10" : ""}`}>
                <div className="font-display text-[42px] font-bold text-red leading-none mb-1.5">{item.stat}</div>
                <div className="font-body text-[15px] text-ink-3 font-light leading-[1.4]">{item.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="how-it-works" ref={s3.ref} className={`py-[100px] px-12 bg-ink relative z-[1] overflow-hidden ${s3.className}`}>
        <div className="max-w-[1100px] mx-auto">
          <SectionRule num="02" title="How It Works" light />
          <h2 className="font-display font-normal leading-[1.2] text-paper mb-[60px] max-w-[600px]" style={{ fontSize: "clamp(32px, 3.5vw, 48px)", letterSpacing: "-0.02em" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-0 border border-paper/10">
            {HOW_STEPS.map((step, i) => (
              <div key={i} className={`px-7 py-8 relative ${i < 3 ? "md:border-r border-paper/10" : ""}`}>
                <div className="font-display text-[56px] font-bold text-paper/[0.06] leading-none mb-4">{step.num}</div>
                <div className="font-mono text-[9px] tracking-[0.14em] uppercase mb-3" style={{ color: "rgba(201,168,76,0.8)" }}>→ {step.label}</div>
                <div className="font-display text-[18px] font-semibold text-paper mb-2.5 leading-[1.3]">{step.title}</div>
                <p className="font-body text-[14px] text-paper/50 leading-[1.6] font-light">{step.body}</p>
                {/* Connector arrow */}
                {i < 3 && (
      ...
      {/* Section 4: Sources */}
      <section id="sources" ref={s4.ref} className={`py-[100px] px-12 max-w-[1100px] mx-auto relative z-[1] ${s4.className}`}>
        <SectionRule num="03" title="Sources" />
        <h2 className="font-display font-normal leading-[1.2] mb-3 max-w-[600px]" style={{ fontSize: "clamp(28px, 3vw, 40px)", letterSpacing: "-0.02em" }}>
          We search where people are <em className="italic text-red">actually honest.</em>
        </h2>
        <p className="font-body text-[16px] text-ink-3 leading-[1.65] font-light max-w-[520px] mb-12">
        ...
        <div className="border border-ink/[0.22]">
          {(Object.keys(sourcesByCategory) as SourceCategory[]).map((cat) => {
            ...
            return (
              <div key={cat} className={`border-b border-ink/10 last:border-b-0 ${isOpen ? "" : ""}`}>
                <button
                  onClick={() => setExpandedCategory(isOpen ? null : cat)}
                  className="w-full flex items-center justify-between px-6 py-4 cursor-pointer bg-transparent border-none text-left transition-colors hover:bg-white/40 select-none"
                >
                  <div className="flex items-center gap-[14px]">
                    <span className="font-mono text-[8px] tracking-[0.12em] uppercase text-red bg-red/[0.06] border border-red/[0.15] px-2 py-[3px] rounded-sm">
                      {cat.replace(/_/g, " ")}
                    </span>
                    <span className="font-display text-[16px] font-semibold text-ink">{SOURCE_CATEGORIES[cat]}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-[9px] text-ink-4 tracking-[0.08em]">{sources.length} sources</span>
                ...
                {isOpen && (
                  <div className="px-6 pb-5 flex flex-wrap gap-2">
                    {sources.map((s) => (
                      <span
                        key={s.key}
                        className="font-mono text-[9px] tracking-[0.06em] text-ink-3 border border-ink/10 rounded-sm px-2.5 py-[5px] bg-white/40 transition-all hover:border-ink-3 hover:text-ink hover:bg-white/80"
                      >
                        {s.display_name}
                      </span>
                    ))}
        ...
        <div className="mt-5 font-mono text-[9px] text-ink-4 tracking-[0.08em] leading-[1.7] border-l-2 border-paper-darker pl-[14px]">
          ...
          A B2B SaaS query routes to G2, HackerNews, and Indie Hackers.<br />
          This routing intelligence is what separates Tethyr from generic search.
        </div>
      </section>

      {/* Section 5: What Makes Tethyr Different */}
      <section ref={s5.ref} className={`py-[100px] px-12 bg-paper-dark border-t border-b border-ink/[0.22] relative z-[1] ${s5.className}`}>
        <div className="max-w-[1100px] mx-auto">
          <SectionRule num="04" title="What Makes Tethyr Different" />
          <h2 className="font-display font-normal leading-[1.2] mb-12 max-w-[600px]" style={{ fontSize: "clamp(28px, 3vw, 40px)", letterSpacing: "-0.02em" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-ink/[0.22]">
            {CLAIMS.map((claim, i) => (
              <div
                ...
                } ${i < 3 ? "border-b border-ink/[0.22]" : ""}`}
              >
                <div className="font-mono text-[9px] tracking-[0.14em] uppercase text-red mb-3">{claim.label}</div>
                <div className="font-display text-[18px] font-semibold text-ink mb-2.5 leading-[1.3]">{claim.title}</div>
                <p className="font-body text-[14.5px] text-ink-3 leading-[1.65] font-light">{claim.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={s6.ref} className={`py-[100px] px-12 max-w-[1100px] mx-auto relative z-[1] ${s6.className}`}>
        <SectionRule num="05" title="What's In Your Report" />
        <h2 className="font-display font-normal leading-[1.2] mb-3" style={{ fontSize: "clamp(28px, 3vw, 40px)", letterSpacing: "-0.02em" }}>
          Five sections. Every claim <em className="italic text-red">evidenced.</em>
        </h2>
        <p className="font-body text-[16px] text-ink-3 leading-[1.65] font-light max-w-[480px] mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-ink/[0.22]">
          {REPORT_SECTIONS.map((sec, i) => (
            <div
              key={sec.num}
              className={`grid grid-cols-[48px_1fr] transition-colors hover:bg-white/50 ${
                i < REPORT_SECTIONS.length - 2 ? "border-b border-ink/10" : ""
              } ${i % 2 === 0 ? "md:border-r border-ink/[0.22]" : ""}`}
            >
              <div className="px-0 py-5 pl-5 font-mono text-[9px] text-red tracking-[0.12em] border-r border-ink/10">
                {sec.num}
              </div>
              <div className="px-5 py-[18px]">
                <div className="font-display text-[16px] font-semibold text-ink mb-[5px]">{sec.title}</div>
                <p className="font-body text-[13.5px] text-ink-3 leading-[1.5] font-light">{sec.desc}</p>
                <span
                  className={`inline-block mt-1.5 font-mono text-[8px] tracking-[0.1em] uppercase px-[7px] py-[2px] rounded-sm ${
                    sec.tier === "free"
      ...
      {/* Section 7: Pricing */}
      <section id="pricing" ref={s7.ref} className={`py-[100px] px-12 bg-paper-dark border-t border-ink/[0.22] relative z-[1] ${s7.className}`}>
        <div className="max-w-[1100px] mx-auto">
          <SectionRule num="06" title="Pricing" />
          <h2 className="font-display font-normal leading-[1.2] mb-12" style={{ fontSize: "clamp(28px, 3vw, 40px)", letterSpacing: "-0.02em" }}>
            Start free. <em className="italic text-red">Pay when it's proven.</em>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-0 border border-ink/[0.22]">
            {PRICING.map((plan, i) => (
              <div
                key={plan.name}
                className={`px-6 py-7 ${i < 3 ? "md:border-r border-ink/[0.22]" : ""} ${
                  plan.dark ? "bg-ink" : ""
                }`}
              >
                <div className={`font-mono text-[9px] tracking-[0.14em] uppercase mb-4 ${plan.dark ? "text-paper/40" : "text-ink-4"}`}>
                  {plan.name}
                </div>
                <div className={`font-display text-[36px] font-bold leading-none mb-1 ${plan.dark ? "text-paper/95" : "text-ink"}`}>
                  {plan.price}
                </div>
                <div className={`font-mono text-[9px] tracking-[0.06em] mb-5 ${plan.dark ? "text-paper/35" : "text-ink-4"}`}>
                  {plan.period}
                </div>
                <div className={`h-px mb-5 ${plan.dark ? "bg-paper/10" : "bg-ink/10"}`} />
                <div className="flex flex-col gap-2 mb-6">
                  {plan.features.map((f, j) => (
                    <div key={j} className={`font-body text-[13.5px] leading-[1.4] flex items-start gap-2 ${plan.dark ? "text-paper/60" : "text-ink-3"}`}>
                      <span className={`font-mono text-[10px] flex-shrink-0 mt-[2px] ${plan.dark ? "text-amber/70" : "text-red-soft"}`}>→</span>
                      {f}
                    </div>
                  ))}
                </div>
                <button
                  className={`w-full py-2.5 rounded-sm font-mono text-[9px] tracking-[0.12em] uppercase cursor-pointer transition-all ${
                    plan.dark
      ...
      {/* Section 8: Final CTA */}
      <section ref={s8.ref} className={`py-[120px] px-12 text-center border-t-2 border-ink relative z-[1] ${s8.className}`}>
        <div className="max-w-[700px] mx-auto">
          <div className="font-mono text-[9px] tracking-[0.2em] uppercase text-ink-4 mb-6">
            Start now · No account needed
          </div>
          <h2 className="font-display font-normal leading-[1.15] mb-5" style={{ fontSize: "clamp(36px, 5vw, 64px)", letterSpacing: "-0.025em" }}>
            Your idea deserves an <em className="italic text-red">honest answer.</em>
          </h2>
          <p className="font-body text-[17px] text-ink-3 leading-[1.7] font-light max-w-[440px] mx-auto mb-10">
      ...
      {/* Footer */}
      <footer className="border-t border-ink/[0.22] px-12 py-6 flex items-center justify-between relative z-[1]">
        <div className="font-display text-[16px] font-bold text-ink">
          Teth<span className="text-red">y</span>r
        </div>
        <div className="font-mono text-[9px] text-ink-4 tracking-[0.08em]">
